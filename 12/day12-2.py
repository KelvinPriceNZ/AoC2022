...
for x, r in enumerate(land_map):
   for y, c in enumerate(r):
      if land_map[x][y].height == "S": start  = Node(x,y,c.height); land_map[x][y].height = chr(ord("a"))
      if land_map[x][y].height == "E": finish = Node(x,y,c.height); land_map[x][y].height = chr(ord("z"))
...
def bfs(begin):
   ...
   #print(f"S: {land_map[begin.x][begin.y]}")
   #print(f"E: {land_map[finish.x][finish.y]}")
   return land_map[finish.x][finish.y].counter
